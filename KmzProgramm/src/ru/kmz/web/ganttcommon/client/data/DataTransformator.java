...
public class DataTransformator implements IDemoData {
	...
	private Task getTask(GraphData data, DateWrapper dw) {
		Task task = new Task(data.getId(), data.getName(), data.getPlanStart(), data.getPlanFinish(), data.getDuration(), data.getDurationWork(),
				data.getComplite(), data.getTaskType(), data.getResourceType());
		for (GraphData child : data.getChilds()) {
